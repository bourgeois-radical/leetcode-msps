...
class Solution:
	def numIslands(self, grid: List[List[str]]) -> int:
		...
		def explore_island_dfs(grid: List[List[str]], cell_i: int, cell_j: int) -> List[List[str]]:
			...
			while stack:
				...
				for _, move_i, move_j in directions:
					...
					if (neighbour_i >= 0 and neighbour_j >= 0) and (neighbour_i < rows and neighbour_j < cols):
